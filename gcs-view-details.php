<?php
   ...
   $xml_1 = new DOMDocument('1.0', 'utf-8');
   $xml_1->load('registeration/documents.xml');
   
   $tags = $xml_1->getElementsByTagName('img');
   
    $doc = array();
   foreach ($tags as $tag) {
          array_push($doc, $tag->getAttribute('src'));
   }
   ...
   foreach ($students as $student) {
     ...
     array_push($progress, $student->getElementsByTagName("progress_status")[0]->childNodes[0]->nodeValue);
                     ...
                     <?php
                        for($i=0; $i < count($student_id); $i++){
                          echo "<tr>
                           ...
                           <td>".$campus[$i]."</td>
                           <td>".$progress[$i]."</td>
                           <td><a href=".$doc[$i]." >view</td>
                           <td><a href=".$doc[$i+1]." target='_blank'>view</td>
                            <td colspan='2'><a <a href=\"gcs-feedback.php?student_id=".$student_id[$i]."\"> GiveFeedback</a> </td>
                           </tr>";
                        }
                        ?>
                  </tbody>
